    ...
    <>
      <nav
        className={`fixed top-0 left-0 right-0 z-[100] flex items-center justify-between w-full transition-all duration-500 px-6 md:px-16 lg:px-24 xl:px-32 ${
          scrolled ? "py-3 mt-3 mx-auto max-w-[92%] rounded-2xl bg-black/80 backdrop-blur-md border border-white/10 shadow-2xl" : "py-6 mt-0 max-w-full"
        }`}
      >
        <Link to="/" className="flex items-center group">
          <img src={logoAura} alt="Aura AI" className="h-10 w-auto min-w-[56px] transition-transform group-hover:scale-110" />
        </Link>

        <div className="hidden md:flex items-center gap-12 text-base font-bold">
          <Link to="/" className="text-white/70 hover:text-white transition-all">Home</Link>
          <Link to="/about" className="text-white/70 hover:text-white transition-all">About</Link>
        </div>
          ...
          {!user ? (
            ...
            <div className="flex items-center gap-4">
              <div className="flex items-center gap-3 bg-white/5 border border-white/10 px-3 py-1.5 rounded-full">
                ...
                <span className="hidden md:block text-white font-bold text-xs">{user.name}</span>
              </div>
              <button
      ...
      {/* Mobile Menu Logic follows the same button pattern */}
      {menuOpen && (
        <div className="fixed inset-0 z-[110] bg-black/95 backdrop-blur-xl flex flex-col p-10 md:hidden">
            ...
            <div className="flex flex-col gap-6 text-xl text-white font-bold">
                ...
                {!user ? (
                    <button onClick={() => { navigate(hasAccount ? "/login" : "/register"); setMenuOpen(false); }} className="text-left py-3 rounded-xl bg-orange-600 px-6">
                        {hasAccount ? "Login" : "Get Started"}
                    </button>
                ) : (
                    <button onClick={() => { logout(); navigate("/"); setMenuOpen(false); }} className="text-left text-red-500">Logout</button>
                )}
            </div>
        </div>
      )}
