      ...
      <section className="flex flex-col items-center text-white text-sm pb-20 px-4 font-poppins">
        <div className="w-full max-w-4xl mt-10 mb-8">
          <div className="flex items-center gap-3 mb-2">
            <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-lime-500 to-green-500 flex items-center justify-center text-lg">✍️</div>
            <h1 className="text-2xl font-semibold">Grammar Fixer</h1>
          </div>
          <p className="text-white/40 text-xs">Paste your text and AI will find and fix grammar, spelling and punctuation errors.</p>
        </div>

        <form onSubmit={onSubmitHandler} className="w-full max-w-4xl flex flex-col gap-4">
          <div className="bg-white/5 border border-white/10 focus-within:border-lime-500/50 rounded-xl p-4 transition-all">
            <div className="flex items-center justify-between mb-3">
              <p className="text-white/30 text-xs uppercase tracking-widest">Your Text</p>
              {inputText && (
          ...
          <button type="submit" disabled={loading || !inputText.trim()}
            className="w-full py-2.5 bg-gradient-to-r from-lime-600 to-green-600 hover:brightness-110 rounded-lg font-medium text-sm transition-all flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed">
        ...
        {checked && (
          <div className="w-full max-w-4xl mt-6 flex flex-col gap-4">
            <div className={`flex items-center justify-between px-5 py-3 rounded-xl border ${matches.length === 0 ? "bg-green-500/10 border-green-500/20" : "bg-red-500/10 border-red-500/20"}`}>
              ...
              {matches.length > 0 && (
                <button onClick={applyAllFixes} className="px-4 py-1.5 bg-lime-600 hover:bg-lime-500 rounded-lg text-xs font-medium transition-all">Apply All Fixes</button>
              )}
            </div>

            {matches.length > 0 && (
              <div className="flex flex-col gap-3">
                <p className="text-white/30 text-xs uppercase tracking-widest">Issues Detected</p>
                {matches.map((match, index) => (
                  <div key={index} className="bg-white/5 border border-red-500/20 rounded-xl px-5 py-4 flex flex-col gap-2">
                    <div className="flex items-start justify-between gap-4">
                      <div>
                        <p className="text-red-400 text-xs font-medium mb-1">{match.rule?.description || "Grammar Issue"}</p>
                        <p className="text-white/60 text-xs leading-relaxed">{match.message}</p>
                      </div>
                      <span className="text-white/20 text-xs shrink-0">#{index + 1}</span>
                    </div>
                    {match.context?.text && (
            ...
            {fixedText && matches.length > 0 && (
              <div className="bg-white/5 border border-white/10 rounded-xl p-4">
                <div className="flex items-center justify-between mb-3">
                  <p className="text-white/30 text-xs uppercase tracking-widest">Fixed Text</p>
                ...
                <p className="text-white/70 text-sm leading-relaxed whitespace-pre-wrap">{fixedText}</p>
              </div>
            )}
          </div>
        )}
        ...
        {!checked && !loading && (
          <div className="w-full max-w-4xl mt-6 bg-white/5 border border-white/10 border-dashed rounded-2xl flex flex-col items-center justify-center h-48 gap-3">
            ...
            <p className="text-white/20 text-xs">Grammar issues and fixes will appear here</p>
          </div>
        )}
      </section>
    </DashboardLayout>
